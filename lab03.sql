...
select *
from "Sales"."Shippers"
where shipperid in (select distinct shipperid from "Sales"."Orders" where
            shipcountry = 'Sweden'
            and shippeddate >= '2006-12-01'::date
            and shippeddate < '2007-01-01'::date
    )
    or shipperid in (
        select distinct shipperid from "Sales"."Orders" where
            shipcountry = 'Denmark'
            and shippeddate >= '2007-12-01'::date
            and shippeddate < '2008-01-01'::date
    );
--
...
select distinct lastname||', '||firstname as "Сотрудник", title as "Должность сотрудника" , c.companyname as "Клиент", c.contactname as "Контакт", c.contacttitle as "Должность контакта"
from "HR"."Employees"e
...
select * 
from "HR"."Employees" where empid not in (select e.empid from "HR"."Employees"e
...
--7.	Выведите информацию о тех товарах, которые никогда не продавались (не вошли ни в один заказ)
 ...
 select p.productname, c.categoryname, s.companyname, p.unitprice , p.discontinued  from "Production"."Products"p
 join "Production"."Categories" c on c.categoryid =p.categoryid 
 join "Production"."Suppliers" s on s.supplierid =p.supplierid 
 where p.productid not in (select od.productid from "Sales"."OrderDetails"od  );
